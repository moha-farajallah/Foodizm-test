 ...
 */
class Atm extends AbstractMethod
{
    /**
     *
     */
    const CODE = 'veritegmo_atm';
    /**
     * @var string
     */
     ...
     * @var string
     */
    protected $_formBlockType = \Veriteworks\Gmo\Block\Form\Atm::class;
    /**
     * @var string
     */
    protected $_infoBlockType = \Veriteworks\Gmo\Block\Info\Atm::class;
    /**
    ...
    public function order(InfoInterface $payment, $amount)
    {
        ...
        $entry->setApiPath('EntryTranPayEasy');
...
//        $this->logger->debug($entry_response, [], true);
        ...
        if (!$this->_handleResponse($entry_response)) {
            ...
            $exec->setApiPath('ExecTranPayEasy');
            ...
            $exec->setParam('PaymentType', 'E');
            $exec->setParam('TelNo', str_replace(
                ["―","－", "－", "-"],
                "",
...
//            $this->logger->debug($exec_response, [], true);
    ...
    public function cancel(InfoInterface $payment)
    {
        ...
        $entry = $this->_connectorFactory->create();
        $entry->setCharset('sjis-win');
        $entry->setApiPath('PayEasyCancel');
        $entry->setParam('ShopID', $this->_gmoHelper->getShopId($storeId));
        $entry->setParam('ShopPass', $this->_gmoHelper->getShopPassword($storeId));
        $entry->setParam('OrderID', $order->getRealOrderId());
